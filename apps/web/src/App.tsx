...
export function App() {
  ...
  useEffect(() => { study.reset(); if (territory.data && isPilotTerritory) study.mutate(); }, [territory.data, isPilotTerritory]);
  ...
  return <div className="v2-app">
    ...
    <main className="v2-main">
      <aside className="decision-panel"><div className="eyebrow">ÉTUDE 01 · ACCÈS AUX SOINS</div><h1>Où implanter le prochain service de santé&nbsp;?</h1><p>Réduire le nombre d’habitants éloignés d’un équipement, en priorisant les secteurs vulnérables.</p>
        ...
        <button className="primary" disabled={!territory.data || !isPilotTerritory || study.isPending} onClick={() => study.mutate()}>{study.isPending ? "Analyse spatiale en cours…" : isPilotTerritory ? "Recalculer les recommandations" : "Territoire à préparer"}</button>{!isPilotTerritory && <small className="territory-notice">● La limite communale peut être explorée. Le diagnostic nécessite encore les équipements, la population carroyée et les contraintes d’implantation de ce territoire.</small>}{study.error && <small className="error">{study.error.message}</small>}{result && <small className="data-status">● {result.data_status}</small>}
      </aside>
      <section className="map-stage">
        {territory.data && <DecisionMap territory={territory.data} result={result} comparison={comparison} visibility={visibility} />}
        <button className="catalog-trigger" onClick={() => setCatalogOpen(!catalogOpen)}>◫ Couches <b>{isPilotTerritory ? Object.values(visibility).filter(Boolean).length : 0}</b></button>
        {catalogOpen && <div className="map-catalog"><header><div><span>CATALOGUE CARTOGRAPHIQUE</span><strong>{isPilotTerritory ? "Données de l’étude" : "Socle métier non chargé"}</strong></div><button onClick={() => setCatalogOpen(false)}>×</button></header>{layerDefinitions.map(layer => <button disabled={!isPilotTerritory} className={isPilotTerritory && visibility[layer.id] ? "visible" : ""} key={layer.id} onClick={() => toggleLayer(layer.id)}><i style={{ background: layer.color }} /><div><strong>{layer.label}</strong><small>{layer.detail}</small></div><b>{isPilotTerritory && visibility[layer.id] ? "ON" : "—"}</b></button>)}<p>{isPilotTerritory ? "Cliquez sur un équipement, une maille ou un candidat pour consulter sa fiche." : "La limite communale est disponible. Les couches décisionnelles seront activées après préparation des sources locales."}</p></div>}
        {isPilotTerritory && <div className="compare-control"><span>Situation actuelle</span><input type="range" min="0" max="100" value={comparison} onChange={event => setComparison(Number(event.target.value))} /><span>Avec site A</span></div>}
      </section>
      <aside className="insight-panel"><div className="eyebrow">IMPACT DU MEILLEUR SCÉNARIO</div>{result ? <><div className="hero-metric"><span>Habitants supplémentaires accessibles</span><strong>+{format.format(result.gained_people)}</strong><small>dans le seuil de {minutes} minutes</small></div><div className="kpi-row"><article><span>Actuel</span><b>{result.current_access_rate}%</b></article><i>→</i><article className="after"><span>Scénario</span><b>{result.scenario_access_rate}%</b></article></div><div className="recommend"><span>RECOMMANDATION N°1</span><strong>Zone A · score relatif {result.recommendation.score}/100</strong><p>{result.recommendation.explanation}</p></div><div className="ranking"><label>ALTERNATIVES</label>{topCandidates.map((feature, index) => <article key={index}><b>{String.fromCharCode(65 + index)}</b><div><strong>Zone candidate {String.fromCharCode(65 + index)}</strong><small>+{format.format(Number(feature.properties?.gained_people ?? 0))} habitants</small></div><em>{String(feature.properties?.score)}/100</em></article>)}</div><div className="ai-card"><span>ASSISTANT EXPLICABLE</span><strong>Interroger l’étude</strong><div>{["Pourquoi cette zone ?", "Qui en bénéficie ?", "Quelles limites ?"].map(item => <button key={item} onClick={() => setQuestion(item)}>{item}</button>)}</div><p><b>Question :</b> {question}</p><p>{question.includes("limites") ? result.limitations[0] : result.recommendation.explanation} <u>Sources vérifiables</u></p></div><button className="report" onClick={() => window.print()}>Générer la note décisionnelle ↗</button></> : !isPilotTerritory ? <div className="territory-empty"><b>Territoire exploratoire</b><strong>{selected.name}</strong><p>Aucun scénario n’est calculé sans données locales vérifiées.</p><ul><li>Équipements métier</li><li>Population carroyée</li><li>Contraintes eau, foncier et urbanisme</li></ul><small>Territoire pilote disponible : Calais · 62193</small></div> : <div className="skeleton">Construction du diagnostic territorial…</div>}</aside>
    </main>
