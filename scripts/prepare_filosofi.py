...
def _normalize(rows: list[dict[str, object]], field: str) -> None:
    ...
    for row in rows:
        row[f"{field}_norm"] = (float(row[field]) - low) / (high - low) if high > low else 0.0
...
def extract_filosofi(zip_path: Path, commune_code: str) -> list[dict[str, object]]:
    ...
    with ZipFile(zip_path) as archive:
        with archive.open("carreaux_200m_met.csv") as raw:
            reader = csv.DictReader(line.decode("utf-8") for line in raw)
            for source in reader:
                if commune_code not in source["lcog_geo"]:
                    continue
...
def to_geojson(rows: list[dict[str, object]], commune_code: str) -> dict[str, object]:
    ...
    for row in rows:
        ...
        geometry = transform(transformer.transform, box(east, north, east + 200, north + 200))
        properties = {
            key: value
...
def main() -> None:
    ...
    population = sum(float(feature["properties"]["population"]) for feature in data["features"])
    imputed = sum(bool(feature["properties"]["imputed"]) for feature in data["features"])
    print(f"{len(data['features'])} carreaux, {population:.0f} habitants, {imputed} imputés")
